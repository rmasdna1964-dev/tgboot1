...
async def business_message_handler(message: Message):
    ...
    if sender_id == owner_id:
        ...
        if lower.startswith(".spam"):
            await handle_spam_command(message, connection_id)
            return

        if lower == ".info":
            await handle_info(message, connection_id)
            return

        if lower == ".starts":
            await start_rps(message.chat.id, connection_id)
            return
        ...
        if lower.startswith(".note"):
            parts = text.split(maxsplit=1)
            if len(parts) < 2:
                await business_send(message.chat.id, "Использование:\n<code>.note текст</code>", connection_id)
                return

            note = parts[1]
            notes.setdefault(message.chat.id, []).append(note)
        ...
        if lower == ".get":
            ...
            if not user_notes:
                ...
                text_out = "📝 <b>Заметки:</b>\n\n" + "\n".join(f"{i + 1}. {note}" for i, note in enumerate(user_notes))

            await business_send(message.chat.id, text_out, connection_id)
            return
    ...
    if ar and ar.get("enabled"):
        ...
        if response_text:
            try:
                await business_send(chat_id, response_text, connection_id)
            except Exception:
                logging.exception("Не удалось отправить автоответчик")
        return
    ...
    if active_trolls.get(chat_id):
        ...
        try:
            await business_send(chat_id, random.choice(troll_messages), connection_id)
        except Exception:
            pass
        return
